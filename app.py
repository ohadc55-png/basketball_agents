...
from pages.drill_library import render_drill_library_page
from pages.practice_planner import render_practice_planner_page
...
def render_login_page(supabase):
    ...
    with col2:
        # Hello Coach banner
        st.markdown('''
        <div style="background: rgba(244, 140, 37, 0.1);
                    border: 1px solid #393028; border-radius: 12px; padding: 1rem;
                    text-align: center; margin-bottom: 1.5rem;">
            <span style="font-family: 'Space Grotesk', sans-serif; font-size: 1.4rem;
                        color: #f48c25; letter-spacing: 2px; font-weight: 600;">
                🏀 HELLO COACH 🏀
            </span>
        </div>
        ''', unsafe_allow_html=True)
...
def render_sidebar(supabase):
    ...
    with st.sidebar:
        # ========== LOGO ==========
        st.image("https://i.postimg.cc/g2QPCwqj/עיצוב_ללא_שם.png", width=160)

        # ========== PROFILE ==========
        st.markdown(f"""
        <div style="padding: 10px 0;">
            <div style="font-size: 15px; font-weight: 600; color: #FFF; font-family: 'Space Grotesk', sans-serif;">{coach.get('name', 'Coach')}</div>
            <div style="font-size: 12px; color: #baab9c; font-family: 'Space Grotesk', sans-serif;">{coach.get('team_name', '')} | {coach.get('age_group', '')} | {coach.get('level', '')}</div>
        </div>
        """, unsafe_allow_html=True)
        ...
        with col1:
            if st.button("💬 CHAT", use_container_width=True, key="nav_chat",
                ...
                st.rerun()
        with col2:
            if st.button("📋 PLAN", use_container_width=True, key="nav_planner",
                        type="primary" if st.session_state.current_page == 'planner' else "secondary"):
        ...
        with col3:
            if st.button("📚 DRILLS", use_container_width=True, key="nav_library",
                ...
                st.rerun()
        with col4:
            if st.button("👥 TEAM", use_container_width=True, key="nav_logistics",
        ...
        if st.session_state.current_page == 'chat':
            ...
            # ========== CHAT HISTORY (3 only) ==========
            st.markdown('<div style="font-size: 13px; color: #f48c25; margin-bottom: 8px; font-family: Space Grotesk, sans-serif; font-weight: 600;">📜 Recent Chats</div>', unsafe_allow_html=True)
            ...
            if conversations:
                ...
                st.markdown('<div style="font-size: 12px; color: #7a6f63;">No chats yet</div>', unsafe_allow_html=True)
            
            st.divider()
            
            # ========== COACHING STAFF - CARDS ==========
            st.markdown('<div style="font-size: 13px; color: #f48c25; margin-bottom: 8px; font-family: Space Grotesk, sans-serif; font-weight: 600;">👥 Coaching Staff</div>', unsafe_allow_html=True)

            staff_data = [
                ("🎯", "Assistant Coach", "Strategy", "#f48c25"),
            ...
            for icon, name, spec, color in staff_data:
                ...
                ">
                    <div style="font-size: 13px; color: #FFF; font-family: Space Grotesk, sans-serif;">{icon} {name}</div>
                    <div style="font-size: 11px; color: #baab9c;">{spec}</div>
                </div>
                '''
        ...
        st.markdown('<div style="text-align: center; font-size: 11px; color: #7a6f63; padding: 8px 0; font-family: Space Grotesk, sans-serif;">Powered by OpenAI</div>', unsafe_allow_html=True)
...
def render_welcome():
    ...
    if not st.session_state.messages:
        ...
        <div class="welcome-banner">
            ...
            <div class="welcome-text" style="margin-top:0.5rem;">Tailored for: <strong style="color:#f48c25;">{coach.get("team_name", "")} | {coach.get("age_group", "")} | {coach.get("level", "")}</strong></div>
            <div class="welcome-text" style="margin-top:0.75rem; font-size:0.85rem; color:#7a6f63;">💡 <em>Check out Quick Ideas in the sidebar for instant prompts!</em></div>
        </div>
        ''', unsafe_allow_html=True)
...
def render_file_upload():
    ...
    st.markdown('''
    <div style="background: #221910; border: 1px solid #393028; border-radius: 12px; padding: 1.25rem; margin: 1rem 0;">
        <div style="font-family: Space Grotesk, sans-serif; color:#f48c25; font-size:1rem; font-weight: 600; margin-bottom:0.5rem;">📁 UPLOAD FILE FOR ANALYSIS</div>
        <div style="color:#baab9c; font-size:0.9rem;">Supported formats: <span style="color:#f48c25;">CSV, Excel, TXT, or Image (Screenshot)</span></div>
    </div>
    ''', unsafe_allow_html=True)
...
def main():
    ...
    if not st.session_state.logged_in:
        ...
        if st.session_state.current_page == 'logistics':
            # Team Manager page
            st.markdown('<div style="text-align:center; padding:0.5rem;"><span style="font-family: Space Grotesk, sans-serif; color:#f48c25; font-size:1.25rem; font-weight: 600;">👥 TEAM MANAGER</span></div>', unsafe_allow_html=True)
            render_logistics_page(supabase, coach.get('id'))
        
        elif st.session_state.current_page == 'library':
            # Drill Library page
            render_drill_library_page(supabase, coach, openai_client)
        
        elif st.session_state.current_page == 'planner':
            # Practice Planner page
            render_practice_planner_page(supabase, coach, openai_client)
