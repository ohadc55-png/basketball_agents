...
def render_calendar(supabase, coach_id):
    ...
    with col2:
        ...
        st.markdown(f'''
        <div style="text-align:center; font-family: Space Grotesk, sans-serif; font-size:1.4rem; color:#f48c25; padding:0.5rem; font-weight: 600;">
            📅 {month_name}
        </div>
    ...
    for i, day_name in enumerate(day_names):
        with header_cols[i]:
            st.markdown(f'<div style="text-align:center; font-weight:600; color:#f48c25; padding:8px; font-size:0.85rem; font-family: Space Grotesk, sans-serif;">{day_name}</div>', unsafe_allow_html=True)
    ...
    # Legend
    st.markdown('''
    <div style="margin-top:1rem; padding:0.75rem; background:#221910; border-radius:10px; border:1px solid rgba(57,48,40,0.8);">
        <span style="color:#f48c25; font-weight:600;">Legend:</span>
...
def render_day_events(supabase, coach_id, selected_date):
    ...
    st.markdown(f'''
    <div style="background:rgba(30,30,30,0.95); border:2px solid #393028; border-radius:15px; padding:1.5rem; margin-top:1rem;">
        <div style="font-family: Space Grotesk, sans-serif; color:#f48c25; font-size:1.1rem; margin-bottom:1rem;">
    ...
    if events:
        for event in events:
            ...
            st.markdown(f'''
            <div style="background:rgba(50,50,50,0.8); border-left:4px solid #393028; padding:1rem; margin:0.5rem 0; border-radius:0 10px 10px 0;">
                ...
                {f"<div style='color:#f48c25;'>vs {event['opponent']} ({event.get('home_away', 'TBD')})</div>" if event.get('opponent') else ""}
            </div>
            ''', unsafe_allow_html=True)
...
def render_event_form(supabase, coach_id, event_id=None):
    ...
    st.markdown(f'''
    <div style="font-family: Space Grotesk, sans-serif; color:#f48c25; font-size:1.2rem; margin-bottom:1rem;">
...
def render_facilities(supabase, coach_id):
    ...
    st.markdown('''
    <div style="font-family: Space Grotesk, sans-serif; color:#f48c25; font-size:1.3rem; margin-bottom:1rem;">
        🏟️ FACILITIES
    </div>
    ''', unsafe_allow_html=True)
    ...
    for facility in facilities:
        with st.container():
            st.markdown(f'''
            <div style="background:linear-gradient(135deg, #221910, rgba(40,40,40,0.8)); 
...
def render_players(supabase, coach_id):
    ...
    for player in players:
        position_color = "#f48c25" if player.get('position') == 'Guard' else "#00D4FF" if player.get('position') == 'Forward' else "#00FF87"
...
def render_logistics_page(supabase, coach_id):
    ...
    <div style="background:linear-gradient(135deg, rgba(20,20,20,0.9), rgba(30,30,30,0.85));
        ...
        <div style="font-family: Space Grotesk, sans-serif; font-size:1.8rem; font-weight:700; color:#9B59B6; margin-bottom:0.5rem;">
            📋 TEAM MANAGER
        </div>
