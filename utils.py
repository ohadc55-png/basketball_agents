...
# ============================================================================
# DATABASE FUNCTIONS - DRILLS
# ============================================================================
def get_drills(supabase, coach_id, category=None, difficulty=None, search=None):
    """Get drills for a coach with optional filters"""
    try:
        query = supabase.table("drills").select("*").eq("coach_id", coach_id)
        ...
        drills = result.data or []
        
        # Filter by search term if provided
        if search and drills:
            search_lower = search.lower()
            drills = [d for d in drills if search_lower in d.get('title', '').lower() 
                      or search_lower in d.get('description', '').lower()]
        
        return drills
    except Exception as e:
        print(f"Error getting drills: {e}")
        return []

def get_drill_by_id(supabase, drill_id):
    """Get a single drill by ID"""
    try:
        result = supabase.table("drills").select("*").eq("id", drill_id).execute()
        if result.data:
            return result.data[0]
        return None
    except Exception:
...
def create_drill(supabase, coach_id, title, description, category, duration_minutes, 
                 difficulty, instructions=None, coaching_points=None, tags=None, is_ai_generated=False):
    """Create a new drill"""
    try:
        data = {
            "coach_id": coach_id,
            ...
            "coaching_points": coaching_points or [],
            "tags": tags or [],
...
def update_drill(supabase, drill_id, data):
    """Update a drill"""
    try:
        result = supabase.table("drills").update(data).eq("id", drill_id).execute()
        if result.data:
            return result.data[0]
        return None
    except Exception as e:
        print(f"Error updating drill: {e}")
        return None

def delete_drill(supabase, drill_id):
    """Delete a drill"""
    try:
        supabase.table("drills").delete().eq("id", drill_id).execute()
        return True
    except Exception:
        return False

# ============================================================================
# DATABASE FUNCTIONS - PRACTICE SESSIONS
# ============================================================================
def get_practice_sessions(supabase, coach_id):
    """Get all practice sessions for a coach"""
    try:
        result = supabase.table("practice_sessions").select("*").eq("coach_id", coach_id).order("date", desc=True).execute()
        return result.data or []
    except Exception as e:
        print(f"Error getting sessions: {e}")
        return []

def get_session_by_id(supabase, session_id):
    """Get a single session by ID"""
    try:
        result = supabase.table("practice_sessions").select("*").eq("id", session_id).execute()
        if result.data:
            return result.data[0]
        return None
    except Exception:
...
def create_practice_session(supabase, coach_id, title, date, total_duration=90, notes=None):
    ...
    try:
        ...
        result = supabase.table("practice_sessions").insert(data).execute()
        if result.data:
            return result.data[0]
        return None
    except Exception as e:
...
def update_practice_session(supabase, session_id, data):
    """Update a practice session"""
    try:
        result = supabase.table("practice_sessions").update(data).eq("id", session_id).execute()
        if result.data:
            return result.data[0]
        return None
    except Exception:
...
def delete_practice_session(supabase, session_id):
    """Delete a practice session and its segments"""
    try:
        # Segments are deleted automatically due to CASCADE
        supabase.table("practice_sessions").delete().eq("id", session_id).execute()
        return True
    except Exception:
        return False

# ============================================================================
# DATABASE FUNCTIONS - SESSION SEGMENTS
# ============================================================================
def get_session_segments(supabase, session_id):
    """Get all segments for a session"""
    try:
        result = supabase.table("session_segments").select("*, drills(*)").eq("session_id", session_id).order("segment_order").execute()
        return result.data or []
    except Exception as e:
        print(f"Error getting segments: {e}")
        return []

def create_segment(supabase, session_id, segment_order, segment_type, title, duration_minutes, drill_id=None, notes=None):
    ...
    try:
        ...
        result = supabase.table("session_segments").insert(data).execute()
        if result.data:
            return result.data[0]
        return None
    except Exception as e:
...
def update_segment(supabase, segment_id, data):
    """Update a segment"""
    try:
        result = supabase.table("session_segments").update(data).eq("id", segment_id).execute()
        if result.data:
            return result.data[0]
        return None
    except Exception:
...
def delete_segment(supabase, segment_id):
    """Delete a segment"""
    try:
        supabase.table("session_segments").delete().eq("id", segment_id).execute()
        return True
    except Exception:
        return False

def reorder_segments(supabase, session_id, segment_ids):
    """Reorder segments by updating their order"""
    try:
        for i, seg_id in enumerate(segment_ids):
            supabase.table("session_segments").update({"segment_order": i}).eq("id", seg_id).execute()
        return True
    except Exception:
        return False